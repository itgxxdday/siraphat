...
def analyze_droplets_core(img, paper_width, paper_height):
    ...
    alpha = 1.8
    beta = 40
    img_adj = cv2.convertScaleAbs(img_enhanced, alpha=alpha, beta=beta)

    # 2. แปลงเป็น HSV และตรวจจับสีเหลือง
    hsv = cv2.cvtColor(img_adj, cv2.COLOR_BGR2HSV)
    ...
    def cv2_to_base64(img_cv):
        # ใช้ JPEG compression เพื่อลดขนาดไฟล์และเพิ่มความเร็ว
